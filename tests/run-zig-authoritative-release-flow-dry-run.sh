...
echo "unexpected gh invocation during dry run: \$*" >&2
exit 1
EOF
chmod +x "$mockbin/gh"
...
PATH="$mockbin:$PATH" \
  bash scripts/run-zig-authoritative-release-flow.sh \
    ...
    --dry-run-output "$dry_run_json" \
    --dry-run \
    >"$flow_log"

grep -Fq "Authoritative Zig flow dry run passed." "$flow_log"
grep -Fq "Ref: main" "$flow_log"
grep -Fq "Expected SHA: $expected_sha" "$flow_log"
grep -Fq "Expected Zig Ref: zigmera/snapshot-v1" "$flow_log"
grep -Fq "Base dir: $tmpdir/archive-root" "$flow_log"
...
if grep -Fq "workflow run" "$gh_log"; then
  echo "dry run should not dispatch the workflow" >&2
  exit 1
fi
